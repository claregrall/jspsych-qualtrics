jsPsych.plugins['audio-slider-response-edit'] = (function () {
  var plugin = {};

  jsPsych.pluginAPI.registerPreload('audio-slider-response-edit', 'stimulus', 'audio');

  plugin.info = {
    name: 'audio-slider-response-edit',
    description: '',
    parameters: {
      stimulus: {
  ...
  plugin.trial = function (display_element, trial) {
    ...
    // var response;  <-- we can remove this because we're now storing an array of responses, rather than a single response

    // for storing the set of slider values sampled every second
    var responses = [];
    // global variable for the response interval timer
    var response_interval;
    var timeOf = [];
    ...
    function setupTrial() {
      ...
      // response = {      <-- we can remove this because the response variable no longer exists
      //    rt: null,
      //    response: null
      // };
      ...
      //   }
      //   // we can remove these two lines because the response variable no longer exists
      //   // response.rt = rt;
    ...
    function end_trial() {
      ...
      // clear the response interval
      clearInterval(response_interval);

      // save data
      var trialdata = {
        responses: responses,  // <-- add new responses array to the data
        timeOf: timeOf,
        //rt: response.rt,          <-- we can remove this because the response variable no longer exists
        startTime: startTime,
        stimulus: trial.stimulus,
        slider_start: trial.slider_start,
        //response: response.response  <-- we can remove this because the response variable no longer exists
      };
